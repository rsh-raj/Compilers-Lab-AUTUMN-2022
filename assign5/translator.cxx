...
// Global Variables
vector<Quad *> quadList;                               // Quad Array
...
Symbol *currentSymbol; // Current Symbol
                       // Counts of number of tables and number of temps generated
...
int SymbolType::getSize()
{
    if (type == CHAR)
        ...
        return __pointer__size;
    else if (type == FLOAT)
        return __float__size;
    else if (type == ARRAY)
...
Symbol *SymbolTable::lookup(string name)
{
    ...
    // If the symbol is not present in the current table, check all the parent tables in the stack
    Symbol *ret_ptr = NULL;
    if (this->parent != NULL)
        ret_ptr = this->parent->lookup(name);

    // if the symbol is not present in any of the table, insert it in the current table and return the symbol
...
void backpatch(list<int> list_, int addr)
{
    ...
    for (auto &i : list_)
    {
        quadList[i - 1]->result = toString(addr);
    }
}
...
void Expression::toInt()
{
    ...
    if (this->type == Expression::typeEnum::BOOLEAN)
    {
        ...
        backpatch(this->trueList, static_cast<int>(quadList.size() + 1));  // update the true list
        emit("=", this->symbol->name, "true");                             // emit the quad
        emit("goto", toString(static_cast<int>(quadList.size() + 2)));     // emit the goto quad
        backpatch(this->falseList, static_cast<int>(quadList.size() + 1)); // update the false list
...
void Expression::toBool()
{
    ...
    {
        // generate symbol of new type and do backpatching and other required operations
        this->falseList = makeList(static_cast<int>(quadList.size() + 1)); // update the falselist
        emit("==", "", this->symbol->name, "0");                           // emit general goto statements
        this->trueList = makeList(static_cast<int>(quadList.size() + 1));  // update the truelist
        emit("goto", "");
    }
}
